...
public class SysDeptServiceImpl extends ServiceImpl<SysDeptMapper, SysDept> implements ISysDeptService
{
    ...
    @Override
    @DataScope(deptAlias = "d")
    public void updateDeptAncerstors(List<SysDept> depts_all)
    {
        for(SysDept d:depts_all){
            List<SysDept> childrens = deptMapper.selectList(Wrappers.<SysDept>lambdaQuery().eq(SysDept::getParentId,d.getDeptId()));
            if(childrens.size()>0){
                SysDept parent = deptMapper.selectDeptById(d.getDeptId());
                for (SysDept children:childrens){
                    children.setAncestors(parent.getAncestors()+","+parent.getDeptId());
                    deptMapper.updateDept(children);
                }
            }
        }
    }
     ...
     * 得到子节点列表
     */
    public List<SysDept> getChildList(List<SysDept> list, SysDept t)
    {
